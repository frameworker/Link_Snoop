...
- (void) windowDidLoad
{
	...
	// Create PDFDocument.
    pdfDoc = [[PDFDocument alloc] initWithURL: [[self document] fileURL]]; // JMN Fix to deprecated fileName
...
- (void) scheduledScanPage: (NSNotification *) notification
{
	...
	if ((annotations != NULL) && ([annotations count] > 0))
	{
		...
		for (i = 0; i < count; i++)
		{
            ...
            NSString *theAnnotationType = [oneAnnotation valueForAnnotationKey:PDFAnnotationKeySubtype];

            if (([theAnnotationType isEqualToString: @"/Link"]) && ([oneAnnotation URL] != NULL)) // WAS @"Link" !?!
			{
				LinkData	*linkDatum;
...
- (id) tableView: (NSTableView *) aTableView objectValueForTableColumn: (NSTableColumn *) theColumn row: (int) rowIndex
{
    ...
    {
        return ([[(PDFAnnotation *)[(LinkData *)[_linkList objectAtIndex: rowIndex] annotation] URL] absoluteString]); // CAST WAS (PDFAnnotationLink *)
    }
    else
		return NULL;
}
